...
def create_sqlite_database(db_file: str = None) -> Connection:
    ...
    with connect(db_file or ":memory:") as conn:
...
def table_select_all(
    ...
    if sort_by:
        ...
        statement_select += f" SORT BY {sort_by} {'ASCENDING' if sort_ascending else 'DESCENDING'}"
...
def table_merge(
    ...
    with conn:
        if into_table:
            ...
            # Make sure the receiving table exists
...
def table_as_csv(
    conn: Connection, table_name: str, output_path: Path = None, **select_opts
) -> None:
